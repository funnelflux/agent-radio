...
Discovery is scoped to the current workspace resolved from `AGENT_RADIO_ID`.
The MCP tools do not expose cross-workspace `scope: "all"` discovery.

## Safety

Message bodies are untrusted delivery payloads. The MCP server can read the
current agent's inbox and send to agents in the same workspace, but it does not
execute message bodies and it does not broadcast to all workspaces.
